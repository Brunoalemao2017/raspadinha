...
if (isset($_POST['aprovar_saque'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT s.*, u.email FROM saques s JOIN usuarios u ON s.user_id = u.id WHERE s.id = ? LIMIT 1 FOR UPDATE");
        ...
        if ($activeGateway === 'ondapay') {
            ...
            $stmt = $pdo->prepare("SELECT client_id, client_secret, url FROM ondapay LIMIT 1");
            $stmt->execute();
            $ondapay = $stmt->fetch();

            if (!$ondapay) {
                throw new Exception("Credenciais OndaPay não configuradas");
            }

            $ci = $ondapay['client_id'];
            $cs = $ondapay['client_secret'];
            // STEP 1: Obter Token (FORÇANDO IPv4)
            $ch = curl_init($ondapay['url'] . '/api/v1/login');
            ...
            if ($httpCode !== 200 || !isset($tokenData['token'])) {
                throw new Exception("Falha ao obter token de acesso da ONDAPAY. HTTP: $httpCode | Response: $tokenResponse");
            }
            ...
            $ch = curl_init($ondapay['url'] . '/api/v1/withdraw/pix');
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_POST, true);
            ...
            if ($httpCode !== 200 && $httpCode !== 201) {
                $errorMsg = "Falha ao processar pagamento na OndaPAY. HTTP: $httpCode | Response: $paymentResponse";


                if ($paymentData && isset($paymentData['msg'])) {
                    $errorMsg .= " | Erros: " . json_encode($paymentData['msg']);
                }
...
<body>
                                ...
                                <?php if ($saque['status'] == 'PENDING'): ?>
                                    <div class="withdrawal-actions">
                                        <form method="POST" style="flex: 1;">
                                            ...
                                            <button type="submit" name="aprovar_saque" class="action-btn btn-approve"
                                                onclick="openLoading()">
                                        ...
                                        <form method="POST" style="flex: 1;">
                                            <input type="hidden" name="saque_id" value="<?= $saque['id'] ?>">
                                            <button type="submit" name="reprovar_saque" class="action-btn btn-reject"
                                                onclick="openLoading()">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            console.log('%c💸 Gerenciamento de Saques carregado!', 'color: #fed000; font-size: 16px; font-weight: bold;');
